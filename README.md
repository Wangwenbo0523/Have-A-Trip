...
| `backend/` | ✅ | 景点列表/详情/搜索、分类标签、来源与许可、行为埋点、推荐接口、AI 能力（一句话检索 / 详情页追问 / 推荐理由润色 / 语义检索 / LLM 行程生成，默认关闭）；214 个用例（206 通过，另 8 个 schema 对拍用例需 PostgreSQL） |
| `frontend/` | ✅ | 首页（推荐位带理由，可 AI 润色）、全部景点（含等级筛选与「用一句话找景点」）、分类页、详情页（含旅游方案、站外视频搜索、就这个景点追问）、帮我排行程（提交 → 轮询 → 结果）、搜索、数据来源与许可页、错误态、页头中英双语切换（默认中文，只翻译界面文案）；82 用例通过 |
