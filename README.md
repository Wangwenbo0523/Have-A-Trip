...
| `backend/` | ✅ | 景点列表/详情/搜索、分类标签、来源与许可、看板数据源、行为埋点、推荐接口、随机抽取、AI 能力（一句话检索 / 详情页追问 / 推荐理由润色 / 语义检索 / LLM 行程生成，默认关闭）；245 个用例（237 通过，另 8 个 schema 对拍用例需 PostgreSQL） |
| `frontend/` | ✅ | 首页（打开时弹出「景区推荐」、推荐位带理由且可 AI 润色）、全部景点（含等级筛选与「用一句话找景点」）、分类页、详情页（含旅游方案、站外视频搜索、就这个景点追问）、帮我排行程（提交 → 轮询 → 结果，可打印行程单）、数据看板、景点对比、搜索、数据来源与许可页、错误态、页头中英双语与深浅色切换（默认中文、默认深色）；116 用例通过 |
...
- **景点对比 `/compare`**：两个下拉各挑一个景点，八行字段并排。选择记在 URL 查询串里（`?a=&b=`），
  对比结果是一条可以直接发给别人的链接。数据走已有的 `/attractions` 与 `/attractions/{slug}`，
  **没有为它开新接口**；只有评分标「更高」—— 票价低、时长长都不等于更好，标出来就是替用户下判断。

- **首页「景区推荐」**：打开首页弹一次，三个景区由 `GET /attractions/random` **随机抽取**
  （与 `/recommendations` 那条按用户缓存的路径不是一回事 —— 这条后端不缓存并显式回 `Cache-Control: no-store`，
  否则随机就成了固定）。名字叫推荐、机制是随机，所以标题下与页脚都写明「随机抽取, 与浏览记录无关」——
...
| GET | `/attractions/random` | 随机抽 `limit` 个已发布景点（默认 3，上限 12）。**不缓存**，每次都不一样 —— 首页弹窗用它 |
