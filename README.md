...
| `backend/` | ✅ | 景点列表/详情/搜索、分类标签、来源与许可、看板数据源、行为埋点、推荐接口、随机抽取与就近推荐（按 IP 猜城市，默认关闭）、AI 能力（一句话检索 / 详情页追问 / 推荐理由润色 / 语义检索 / LLM 行程生成，默认关闭）、用户动态区（发动态 / 按景点或按设备看 / 作者删除，匿名无定位）；363 个用例（353 通过，另 10 个 schema 对拍用例需 PostgreSQL） |
...
界面走的是**白画布 + 单一品牌色 + 柔和圆角、照片优先**的一套语言，选型来自 awesome-design-md 的
Airbnb 一份。令牌表、组件规范与 Do/Don't 全写在 **[`DESIGN.md`](DESIGN.md)** —— 改界面照着它改，不照着猜。

> 只借公开描述的设计语言（配色关系、圆角刻度、层级策略），没有使用对方的字体文件、图标、图片或字标。

## 体验与可视化

四件事，共同点是**前端不编造任何数据**，且**没有引入任何新依赖**（许可证卡口仍是零违禁）：

- **深浅色主题**：切换按钮在页头右上角、与语种按钮并排，按钮上写的是**目标主题**的名字。默认**浅色**，
  不嗅探 `prefers-color-scheme`（与语种不嗅探 `navigator.language` 同一口径），选择存在
  `localStorage["have-a-trip:theme"]`。两套主题共用同一份组件样式 —— 深色只覆盖 `frontend/src/index.css`
  里的**原生色板**，不碰尺寸与布局；`index.html` 有一段内联脚本在 React 挂载前把主题写到
  `<html data-theme>`，否则选了深色的用户每次刷新都会先看到一闪的白底。
...
| 路径 | 说明 |
|---|---|
| `DESIGN.md` | 界面设计系统（配色 / 圆角 / 字号 / 层级 / 组件规范）：改界面前先读它，见「设计系统」 |
...
| `-Port 8101` | 换端口 |
| `-DatabaseUrl ...` | 直接指定库；不给就按 dev-up 那套探测（`DATABASE_URL` → 5432 → 55432）。**探不到任何库就不启动**（起了也是每个接口都 500），并把便携实例的起库命令打印出来 |
...
3. **图片同样算资产**——页头那张来源无从查证的实景照片已经删掉（页头现在是白底 + 一条细线，连那段 CSS 渐变也没有了），见 `docs/LICENSE-AUDIT.md` 第五节。
