...
| `frontend/` | ✅ | 首页（打开时弹出「景区推荐」、推荐位带理由且可 AI 润色）、全部景点（含等级筛选与「用一句话找景点」）、分类页、详情页（含旅游方案、站外视频搜索、就这个景点追问）、帮我排行程（提交 → 轮询 → 结果，可打印行程单）、数据看板、景点对比、搜索、数据来源与许可页、错误态、页头中英双语与深浅色切换（默认中文、默认深色）；128 用例通过 |
