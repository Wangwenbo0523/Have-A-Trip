...
### 本项目当前的数据状况（2026-09-25，随每次种子扩容同步）

| 项 | 现状 |
|---|---|
| 种子数据 | 156 个景点（中国境内 116 + 境外 40，覆盖六大洲 30 个国家）、32 个标签、552 条标签关联，全部自采 |
| 旅游方案 | 156 个方案、506 条步骤，同样是自采的行程建议（不是官方或旅行社线路） |
| 等级字段 | `a_level` 50 条、`heritage` 63 条，**逐条查证后才填**，查不到就留空 |
| `source` | `Have-A-Trip 自采（公开事实信息）`（156 / 156 条） |
| `license` | `MIT`（156 / 156 条） |
| 第三方数据集 | **一个都没有** —— 本节表格里那些源，至今一个没用；v2.0 还主动放弃了一份 CC BY-SA 的 5A 名录（见上表） |
| 图片 | `attraction_image` 156 行，全部是 `scripts/make_attraction_covers.py` 自绘的 SVG |
