...
| 2026-09-26 | v4.2 | **把中国 A 级景区名录编进源文件（种子 156 → 1385）**：需求是「查找中国所有 3A / 4A / 5A 级景区…将数据编入源文件中, 加大资源」。数据只取**政府公开名录**, 三条：文化和旅游部旅游服务平台的「全国 5A 级旅游景区」名录（359 条, 分页接口 `/api/marker/list?type_id=1`）、广东省文旅厅《广东省 A 级旅游景区名录表（截至 2026-08-31）》（754 家）、北京市文旅局《北京旅游网景点链接列表》（205 家）。**为什么不用现成的第三方数据集**：`docs/LICENSE-AUDIT.md` 第三节早就拒绝过 `tuansuwu/china-5a-scenic-areas`（CC-BY-SA-4.0）—— share-alike 会顺着数据传染, 与「将来可闭源」冲突; 求闻百科的分省 4A / 5A 快照同样是 CC BY-SA, 一并不用。所以这一版的取舍是**宁可 4A / 3A 只覆盖广东与北京, 也不把 copyleft 引进来**（5A 反而是全国完整的 331 条）。新增 `db/seed/data/cn_a_level.csv`（1229 行）+ `db/seed/data/README.md`（来源、字段、合并规则、已知缺口）+ `scripts/build_cn_attractions.py`（CSV → `db/seed/attractions_cn.sql`, 带 `--check`）+ `db/seed/attractions_cn.sql`（生成物, 与逐条审过的 `seed.sql` 分开, 免得两边都难读）。合并三条规则：① 省级名录优先且独占该省级行政区（文旅部跳过广东 16 / 北京 9 共 25 条 —— 北京 5A 记 10 家, 文旅部把「八达岭—慕田峪长城」并作 1 家）；② 与 `seed.sql` 的自采景点按「核心名 + 省级行政区」去重（剔除 30 条, 其中 29 条 5A —— 「杭州市西湖风景名胜区」不会和自采的「西湖」重复、「三亚市天涯海角游览区」不会和「天涯海角游览区」并存；「核心名」= 去掉开头的城市前缀与末尾的「景区 / 风景区 / 风景名胜区 / 旅游度假区 / 博物馆 / 公园…」）；③ 同级同名同省再去一次。刻意不收：广东 11 家 2A、北京 21 家 2A 与 2 家 1A（`a_level` 的 CHECK 只到 3A, 写成空值会与「留空 = 未核实」冲突）, 以及文旅部接口里的 `introduce` 成篇简介与经纬度（简介的著作权状态跟一份名单不是一回事；坐标与「一期不做地图与定位」的口径冲突）。新增第 10 个分类 `scenic-area`「A 级景区」—— 名录里没有主题分类, 猜一个不如老实开一格。这批条目**没有简介、没有标签、没有方案**, 所以 `db-schema.yml` 里「每个景点都要有标签 / 都要有方案」两条按 `source` 收口到自采那 156 条, 另加「官方名录 1229 条」「自采 156 条」两条正面断言, 并把 `category` 9→10、`attraction` 156→1385、`attraction_image` 156→1385、`a_level` 50→1279、重跑后景点数 156→1385 一并改掉；重跑自愈那段加了名录条目的用例（把 `5a-shanghai-001` 的名称 / 等级 / 状态 / 许可改坏, 重跑 `attractions_cn.sql` 必须修回来）。`scripts/make_attraction_covers.py` 跟着扩：解析器改成认两个种子文件、并支持 `name_en` 为 `NULL` 与名字里的转义单引号 —— 广东有一家「广州市北纬23°8'森林营地旅游区」, 旧正则遇到 `''` 会把整行漏掉（首轮生成 1385 张里少了它, 靠对账才发现）, 新增 `scenic-area` 的配色与分类名；封面 156 → **1385** 张（约 2.9 MB）与 `images.sql` 同步。文档：`db/README.md`（文件表、执行顺序、行数、新增「官方名录条目」一节、数据来源清单表 1 行 → 4 行）、`docs/LICENSE-AUDIT.md`（第三节表把「官方文旅部门公开名录、自行采集」拆成两行并改成「在用」, 当前数据状况表整体改写）；前后端 6 处「库里 156 条」的注释改成 1385（`AttractionPicker.tsx` / `ComparePage.tsx` / `picker.css` / `ComparePage.test.tsx` / `backend/app/api/attractions.py` / `backend/tests/test_attractions.py`）。验证：`pglast` 解析 `schema.sql` / `seed.sql` / `attractions_cn.sql` 通过（11 / 11 / 4 条语句；**pglast 自己是 GPL-3.0, 用完要卸掉, 否则本地 `license_gate --strict` 会红**, 这条已写进 `db/README.md`）；`build_cn_attractions.py --check` 与 `make_attraction_covers.py --check` exit 0；`license_gate.py --strict` 通过（15 项既有告警不变）；后端 361 条用例全过（10 跳过）、`tsc --noEmit` exit 0、前端两个相关测试文件 22 条全过。**行为层没在本机验**：这台机器没有 `psql`、没有 Docker, 行数与 CHECK 只能靠 CI 的 `db-schema.yml`（起 `postgres:16` 把三个种子文件各跑两遍再断言）。 |
| 2026-09-26 | v4.3 | **CI 卡口补齐与「不静默丢条目」（借鉴 jianhao-travel-planner）**：翻 GitHub 上 `awangwang123/jianhao-travel-planner`（MIT，出行路书 skill，22 星）的做法, 挑出三处能落地的。① 它的 `tools/consistency.py` 立的是「拿基准骨架当唯一真值, 产物必须与基准逐字节相等, 不手抄指纹数字」——本项目 v4.2 写出了 `build_cn_attractions.py --check`, 却**没接进任何工作流**（`license-gate.yml` 里原本只有 favicon 与景点封面两道生成物闸）, 生成物校验脚本存在却不生效, 是同一类问题; 补上「校验名录种子 SQL 与源 CSV 一致」一步 —— `attractions_cn.sql` 是直接喂给 psql 的, 这道锁比封面那道更要紧。② 它的 `tools/desource.py` 每次替换都断言「期望命中 N 处, 实际 M 处」, 不许静默跳过 —— `make_attraction_covers.py` 的解析器原本会**静默丢掉**「认出景点元组却取不到分类」的条目（丢一条 = 该景点没有封面, 而封面缺失在界面上只表现为一张空白首图, 不报任何错, 是最难回头发现的一类回归）, 现在连「slug 重复」（会被 `files` 这个 dict 与 `images.sql` 的 upsert 静默吃掉）一起变成硬错误。③ 新增 `scripts/verify_all.py`：把本机能跑的静态门禁（许可证 / 站点图标 / 景点封面 / 名录 SQL / 工作流可解析）收成一个入口, 出「通过 / 跳过 / 失败」汇总, 并**在末尾逐条列出没覆盖的检查**（db-schema 的结构与行数断言要真实 PostgreSQL、pytest 与前端三项要装依赖且以分钟计、前端守线的判据只在 `frontend-build.yml` 里不抄第二份、DCO 在 push 到 main 时只告警）, 免得「这里过了」被当成「全过了」; `CONTRIBUTING.md` 第五节与 `docs/DEPLOY.md` 检查清单同步指向它。**刻意不借鉴的两点**：一是 `db-schema.yml` 里 1385 / 1229 / 156 这几个行数, jianhao 的做法是从源派生、不手抄; 本项目这几个数字是**人工确认过的口径总账**（改种子就该有人再确认一次规模）, 所以保留手写, 把「静默漂移」交给 `--check` 挡 —— 一致性由 `--check` 保证, 数字只作独立对账。二是它的六维度路书（吃 / 玩 / 住 / 行 / 拍 + 避雷）与美食情报卡, 那是**内容形态**: 与本项目「库里没有的宁可不说、模型只做编排不做知识源」的口径（`backend/app/trip/prompt.py` 的硬约束: 不写票价 / 开放时间 / 距离 / 交通耗时 / 天气）正面冲突, 搬进来只会给编造开口子。验证：`python scripts/verify_all.py` exit 0（5 项全通过）; `make_attraction_covers.py --check` 与 `build_cn_attractions.py --check` exit 0; 新护栏用临时副本实测会拦（「取不到分类」报 `a.sql:98 认出了景点元组 'west-lake' 却找不到分类`、「slug 重复 2 次: west-lake」, 两者均 exit 1）; `license_gate.py --strict` 通过（15 项既有告警不变）。踩坑记一条：`verify_all.py` 第一版把子进程输出按 UTF-8 解、子进程却按 cp936 写, 解出来的乱码里含有 GBK 编不回去的字符, 直接把脚本打崩 —— 子进程必须钉 `PYTHONIOENCODING=utf-8`（jianhao 那个 `_SYM_FALLBACK` 兜底想解决的正是这类问题, 本项目靠钉编码从根上避开, 也不引 emoji）。 |
| 2026-09-26 | v4.4 | **打一个能直接发给别人的「简易安装包」**：此前所有入口都默认对方有 Node（要 build 前端）与一套命令行习惯，v4.3 借鉴 `jianhao-travel-planner` 时留下的那半件事在这轮补齐 —— 给「只想用」的人一条双击就完的路。三件新东西：`scripts/make_installer.ps1`（打包）、`installer/install.ps1`（装配）、`install.cmd`（双击入口）。**包里装什么**：git 跟踪的文件 + 预构建的 `frontend/dist`（这是包里唯一比仓库多的东西，也是拿包的人**不需要 Node** 的原因）；装的是「用」的形态，落 `%LOCALAPPDATA%\Programs\Have-A-Trip`（用户级，免管理员）。**为什么按 `git ls-files` 取而不扫工作区**：第一次试跑就是扫工作区，结果把上次会话遗留的 `.dev-q1.out` / `.dev-q2.sql` 等 6 个探针文件打进了包 —— 扫工作区没有「该不该进仓库」这个概念；改成问 git 之后这类东西永远进不来，代价是新文件必须先 `git add`，缺关键文件时脚本按 `$mustHave` 清单硬失败，宁可不出包也不出残包。**还有一道「产物比源旧就停」**：vite 把源在构建期写死进产物，`frontend/dist` 只要比 `frontend/src` / `frontend/public` 旧就硬失败 —— 这道卡口不是想出来的，是**真踩到之后补的**：本机那份 dist 是 v4.2 加景区封面之前建的，用它打出来的第一个包前端少了 1385 张封面（1766 个文件，正确应是 2995 个），界面上是整片破图，而「装上了、看着像好的、只是图全没了」正是最难回头发现的一类错。**为什么不用 `Compress-Archive`**：两处不合用 —— 通配符 `-Path` 看不见隐藏项（`.gitignore` / `.env.example` / `.gitattributes` 会静默漏掉，包里就少了几个正常文件），而 .NET Framework 下条目名会写成反斜杠（非 Windows 解压出一堆怪名字）；改成 `ZipFile` 逐条用正斜杠写。**打包脚本自己验包**：打完把 zip 的目录表读一遍，缺 `install.cmd` / `installer/install.ps1` / `frontend/dist/index.html` / `db/seed/attractions_cn.sql`，或混进 `.git` / `.dev*` / `景区名录` / `node_modules` / `.venv` / `.env`，就**删掉 zip 并 exit 1** —— 「发出去了」是最贵的失败。**安装脚本**：复制文件 → 建 `backend\.venv` 装 `requirements.txt`（运行时依赖，不是 dev）→ `createdb` 后按 `schema → seed → attractions_cn → images` 灌 → 建桌面与开始菜单快捷方式；不代装 Python / PostgreSQL，缺哪个就停下来说清去哪装（与 `dev-up.ps1` 同一条口径，不下载不安装不瞎猜）；`PYTHONUTF8=1` 挡中文控制台（代码页 936）下 pip 的 UnicodeDecodeError；`backend\.env` 只在不存在时由 `.env.example` 生成且**不带 BOM**（带 BOM 会让第一个键名多出不可见字符），已存在一律不动；卸载得删掉脚本自己所在的目录，办法是丢一个影子脚本到 `%TEMP%` 等两秒再动手。`install.cmd` 与 `damo.cmd` 一样纯 ASCII —— cmd.exe 按 OEM 代码页读 .cmd，中文在 936 上就是乱码。**顺带修掉的真遗漏**（查文档一致性时发现的，性质与 v4.2 那批相同）：`scripts/dev-up.ps1` 与 `scripts/demo-offline.ps1` 只灌 `seed.sql` + `images.sql`，**漏了 `attractions_cn.sql`** —— 用这两个脚本起的本机库只有 156 条而不是 1385 条，而 v4.2 之后界面、文档、CI 都按 1385 说话；`README.md`「手工起」与 `docs/DEPLOY.md`「二、数据库」的灌库清单也各漏一处，四处都补上，并在 dev-up 与 DEPLOY 写明**顺序不能换**（`images.sql` 按 slug 关联两个景点种子）。文档：README 目录表加 `installer/` 与 `install.cmd` 两行、`scripts/` 一行补打包脚本、快速起步加「做安装包（发给别人）」一节；DEPLOY 加「四·六」。验证：`python scripts/verify_all.py` exit 0（5/5）；`make_installer.ps1` 出 `have-a-trip-0.1.0.zip`，2995 个文件 / 7.1 MB（仓库跟踪 1604 + `frontend/dist` 1390），SHA256 与文件数打在结尾，重跑会先清掉旧 zip 与旧暂存目录；把 zip **解压**出来独立查一遍：2995 个文件 / 12.1 MB，其中 `frontend/dist` 1390 个（含 `images/covers` 那 1385 张封面），顶层没有 `.git` / `node_modules` / `.venv` / `.dev-q*.out` / `景区名录` / `.env`，`install.cmd`、`installer\install.ps1`、`frontend\dist\index.html`、`db\seed\attractions_cn.sql`、`安装说明.txt` 都在；拿解压出来的那份真装两遍（`-SkipDeps -SkipDatabase -SkipShortcuts` 只验复制与落位，以及 `-PgBin C:\pgtemp\pginstall\bin -PgPort 55432 -Database have_a_trip_inst_test` 连便携实例走完整数据库路径）：建库 → 四个 SQL → **库内景点数 1385** → 生成 `backend\.env`（无 BOM，`DATABASE_URL` 指到新库），跑完把测试库 drop 掉；最后用仓库 venv 的解释器、以**装出来的那一份**（cwd 为安装目录的 `backend`，`SERVE_FRONTEND=true`、`FRONTEND_DIST` 指安装目录的 `dist`）跑一遍 `TestClient`：`/api/v1/attractions` 200 且 `total=1385`、`/` 200 且回来的是 `dist` 的壳、`/api/v1/ai/status` 200 且 `available=false`（没配模型就如实降级，不报错）。 |
| 2026-09-26 | v4.5 | **5A 景区配图换成实景照片（130 张, 抓自 Wikimedia Commons）**：需求是「没有配图，AI 生成快还是网上截取快？」→ 选 B（真照片）→「存仓库，先不管是否违规，先把图片标好」。**先解决网络**：Wikimedia 直连时 TLS 握手就被重置，本机又没有通用 HTTP 代理（`127.0.0.1:7890` 空着，雷神 / AK 加速器只代理 hosts 名单里的域名），于是从 GitHub 上公开的代理列表（monosans/proxy-list、TheSpeedX/PROXY-List，经可达的 `api.github.com` 取回）扫了 5778 个候选，实测出 222 个能 CONNECT 到 Wikimedia 的；抓取一律用默认 TLS 上下文校验证书链，中间人代理会握手失败 —— 所以拿到的字节确实来自 Wikimedia。**只搜 Commons**：`commons.wikimedia.org` 的 `action=query&generator=search`（`filetype:bitmap`，缩略图现在走 `thumb.wikimedia.org`）；中文维基的首图精度更高但每批要 86~93 秒，放弃。**名字变体是关键**：名录里是全称（丽江市玉龙雪山景区），Commons 文件名常用核心名（玉龙雪山），加「去城市前缀 / 去景区后缀」四种变体后，5A 命中率从 12% 提到 37%。**质量过滤**：许可白名单（PD / CC0 / CC BY / CC BY-SA，排 NC / ND / 非自由）、宽 ≥ 1000px、剔除卫星图 / 地图 / 标志 / 画作，以及**机场 / 医院 / 车站 / 停车场 / 收费站** —— 最后这组是把关正则漏了 `re.I` 时真放进来的错图（华山配成「华山医院」、稻城亚丁与长白山配成机场、千岛湖配成火车站），第二批才补上。结果：5A 373 条命中 130 张（37%），4A 抽检 12 条命中 9 张，3A 冷门基本白跑。**怎么落**：新增 `db/seed/photos.json` 台账（逐张记 pid / seq / slug / photo / file / license / artist / source / width / height）与 `frontend/public/images/covers/*.jpg`（800px，130 张 23.1 MB，与 1385 张自绘 SVG 并存）；`scripts/make_attraction_covers.py` 改成「有照片用照片、没有就自绘」，`caption` 带 `#001` 形式的编号 —— 这就是需求里的「把图片标好」：编号在台账、图集 `caption`、images.sql 三处一致，可以对着样张逐张核。`db/seed/images.sql` 新增一条 `DELETE FROM attraction_image WHERE url LIKE '/images/covers/%'`：不清旧行的话，某景点从自绘换成照片后会留下两行 `sort = 0`，`cover_image` 指哪张全看运气。工具留档：新增 `scripts/fetch_commons_photos.py`（搜索 → 打分 → 下载 → 写台账，带 `--check`；等级过滤用 `db/seed/data/cn_a_level.csv` + `seed.sql` 的等级映射表离线算，不依赖数据库），理由与自绘封面一样：留脚本，出处才查得到。**已知合规缺口（当时明确「先不管」）**：130 张里 98 张 CC BY-SA、27 张 CC BY —— 署名是硬义务，CC BY-SA 还要求相同方式共享，而界面只有按「作者 + 许可」聚合的几行，没有逐图署名与来源页链接。已如实写进 `docs/LICENSE-AUDIT.md` 第五节的表格、原则与待办，`share-alike` 那一行也从「无」改成「图片资产有」。**验证**：`make_attraction_covers.py --check` 一致（1385 张封面 + `db/seed/images.sql`）、`fetch_commons_photos.py --check` 130 条与磁盘一致、`verify_all.py` 5/5、`license_gate.py --strict` exit 0；灌库后 `attraction_image` 仍是 **1385 行 = 每个景点恰好一行**（1255 条指向自绘 SVG、130 条指向实景照片），`sort = 0` 多于一行或一行为空的景点都是 0 个，`cover_image` 指 JPG 的 130 个、指 SVG 的 1255 个；抽 40 张一页做了 4 张样张人工过目，约 3/4 是好图（黄山 / 九寨沟 / 张家界 / 泰山 / 长城 / 故宫 / 布达拉宫 / 平遥 / 云冈 / 龙门），约 1/4 偏弱（博物馆大门、雕像、施工中的橘子洲）。文档：README 的数据量与 `scripts/` 目录表、db/README 的表清单与「刻意不写的数据」、LICENSE-AUDIT 第三 / 第五节同步 |
| 2026-09-26 | v4.6 | **加一个「命令行到底走不走代理」的体检工具**：接 v4.5 抓图那轮 —— 当时为了查清 `127.0.0.1:7890` 为什么是空的，全靠临时手写注册表查询，这轮把结论固化成 `scripts/system_proxy.py`。它把这件最容易混的事分三层说清：**WinINET**（「系统代理」开关写的地方，多数 GUI 程序用）与 **WinHTTP**（系统服务与部分命令行工具用）是两套互不相通的设置；WinINET 里又分固定代理与 **PAC 脚本**，而 Python 与 curl 都不执行 PAC —— 所以 PAC 模式下的表现就是「浏览器能上、脚本上不了」；Python 只看环境变量与 WinINET 固定代理，且**环境变量优先**（设了任意一个就完全不再看注册表）。默认**只读**注册表与环境变量、不改任何设置；`--test` 才发一次真实请求（默认打 Wikimedia 的 API —— 国内典型必须走代理的站点，一次请求就能分清「代理生效」还是「在直连」）；`--export powershell|cmd|sh` 打印可直接粘的临时环境变量；`--json` 给别的脚本吃。退出码分四种（0 正常 / 1 没找到代理 / 2 平台不对 / 3 有代理但连不通）。文档：README 的 `scripts/` 目录表加一行，`fetch_commons_photos.py` 的文件头指向它（抓图前先看一眼）。顺带清掉仓库根遗留的 6 个 `.dev-q*.out` / `.dev-q*.sql` 探针文件（更早会话调试留下的；v4.4 打包时正是它们暴露了「扫工作区会把不该进仓库的东西打进包」）。验证：本机跑一遍，如实报出三处都没配代理、命令行走直连，exit 1 |
| 2026-09-26 | v4.7 | **修「数据库没起时启动出一个每个接口都 500 的空壳」**：现象是双击 damo 打开首页, 「为你推荐」「按分类浏览」「最新收录」整片报「后端返回 500 / 分类还没加载出来」。查下来根因既不在前端也不在接口: 便携 PostgreSQL 实例上次是**非正常停止**的(`postmaster.pid` 还留在 `C:\pgtemp\pgdata`、进程早没了, `server.log` 里是 `database system was not properly shut down; automatic recovery in progress`), 于是 `damo-app.ps1` 的端口探测(5432 → 55432)两个都不通, **却仍然退回默认的 5432 把服务起了起来**; 而 `/api/v1/healthz` 在数据库连不上时是**有意**回 200 + `status=degraded`(探活接口自己不该挂), 启动器只看「有没有 status 字段」, 于是一个坏掉的实例被判定成就绪、窗口照开 —— 界面上没有任何一处说「数据库没起」。修法两条: ① **就绪判定只说真话**: 收进 `Test-Healthy`, 只认 `status -eq "ok"`; 等就绪、复用已有实例、起来之后复核, 三处都走它 —— 复用分支遇到 degraded 就弹框说清并要求 `-Down` 重开, 起来后仍连不上库就**收掉实例**退出, 不留「看着像好的」窗口; ② **探不到库就不启动**(与 `dev-up.ps1` 早就有的硬失败对齐), 并用新增的 `Get-DbHint` 把便携实例的 `pg_ctl` 起库命令原样打出来。测试: `test_launcher.py` 5 → **7** 条(「不能把 degraded 当就绪」「数据库没起必须硬失败且给出 pg_ctl 命令」), 并做了负向控制 —— 把等就绪那行退回旧的 `-not (Get-Health)`, 新用例立刻红并指出是哪一行, 恢复后按字节核对 sha256 一致。验证: 起库 → `damo.cmd -Down` → 重开, `/api/v1/healthz` 回 `{"status":"ok","database":"ok"}`, `/api/v1/categories`、`/attractions`、`/sources`、`/recommendations` 四个原本 500 的接口全 200 且带数据, 首页截图确认推荐卡片 / 分类 / 最新收录都回来了; 后端 363 条用例全绿(10 条对拍跳过)。文档: README 的 `-DatabaseUrl` 一行写明「探不到库就不启动」, 后端用例数 361 → 363 |
| 2026-09-26 | v4.8 | **界面按 Airbnb 的设计语言重做**：`awesome-design-md` 里 74 份品牌设计系统，选了 airbnb 一份（MIT）—— 本站是旅游目录、库里有 130 张实景照片，正好是那套语言服务的场景（白画布、单一品牌色、柔和圆角、照片优先）。① **令牌分三层**（原生色板 `--canvas/--ink/--primary` → 分块取色 `--page-bg/--text/--accent` → 组件只用分块），全部收进 `frontend/src/index.css`；**深色主题只覆盖原生色板**，所以组件样式一行没改就跟着换；② **默认主题从深色改成浅色**（白画布优先是这套语言的底色），`theme/index.tsx`、`index.html` 的 `theme-color` 与两处测试同步；③ **页头重做**：白底吸顶 + 一条细线，品牌名用 Rausch（全站唯一一处品牌色文字），导航当前页是**墨色下划线 + 加粗**（不再是橙色胶囊底），语种/主题按钮回到 flex 正常流（原先绝对定位到右上角，窄屏会打架），并删掉 `Staatliches` 的 Google Fonts 外链；④ **卡片改成照片优先**：4:3 打头、14px 圆角、1px 细线、悬停只浮起**唯一那一档**阴影（不再把边框刷成橙色），分类角标从深色遮罩块改成左上角白胶囊；⑤ **评级徽章收进同一套色**：5A 用 Rausch 实底、4A 墨色实底、3A 白底墨字、世界遗产白底加墨色描边 —— 删掉原先的金色 / 青色渐变；⑥ 筛选选中态与次级按钮改墨色实底（一屏好几组筛选项时品牌色会失去重点）；顺带把 `index.html` 里那段「首屏定主题」内联脚本从 `</html>` 之后**挪回 `<head>`**（原先那是一段无效 HTML，靠浏览器容错才生效）。新增 **`DESIGN.md`**（令牌表、组件规范、Do 与 Don't、改完的验证步骤），README 加「设计系统」一节与目录表一行。**验证**：前端 157 用例全绿、`tsc --noEmit` 0 错、`vite build` 通过；起真实例（库 + `damo-app.ps1`）后用 Edge 截图过首页 / 全部景点 / 详情页，另用 CDP 驱动 Edge 切到深色再截一张，两套主题都成立。**注意**：`frontend/dist` 已重建（本机应用吃的是它） |
| 2026-09-26 | v4.9 | **重打安装包、用真包实装，装出三个真 bug 全修**：这次不是「脚本写完就算」，而是把 zip 解到一个干净目录、照拿包人的流程双击 `install.cmd` 一路走到底。① **`Invoke-Exe` 的返回值一直是个数组**：`installer/install.ps1` 里写的是裸 `& $Exe @Arguments`，而 PowerShell 会把子进程写到 stdout 的每一行都当函数**返回值**，于是 `$rc = Invoke-Exe ...` 拿到的是「一整屏输出 + 退出码」，`$rc -ne 0` **恒真** —— pip 明明装成功，脚本却报「pip 装依赖失败」并中止。robocopy / psql / `python -m venv` 一直没露馅，只因为它们的 stdout 几乎是空的；**换句话说这条路一直是坏的，只是从没人在一台没有 `backend\.venv` 的机器上走到过**。改成 `& $Exe @Arguments 2>&1 \| Out-Host`（转发到控制台但不进管道），并把这段调用期间的 `$ErrorActionPreference` 降到 `Continue`、`finally` 还原，仍 `return $LASTEXITCODE`。② **`install.cmd` 的失败分支自己先崩了**：那句 echo 里带一个裸 `)`，提前闭合了 `if (...)` 块，cmd 直接报 `. was unexpected at this time.`，而报错点就在**失败分支**里 —— 真正的失败原因被整个盖掉；拆成两行、去掉括号。③ **「连不上数据库」的提示永远打不出来**：探测连库写的是 `$probe = & $psql ... 2>&1`，psql 连不上时往 stderr 写一行，合流后触发脚本顶上的 `$ErrorActionPreference=Stop` 升级成**终止错误**，紧接着那两行专门写给人看的提示（「确认 PostgreSQL 服务在跑、端口对、口令对」）一个字都出不来 —— 而「库没起」恰恰是这条路上最常见的失败；改走 `Invoke-Exe`，失败时报 psql 退出码 + 原有提示。新增 `backend/tests/test_installer.py` **7 条**（installer 下 `.ps1` 必须带 BOM、`.cmd` 纯 ASCII、echo 行不许有裸 `)`、`Invoke-Exe` 不许裸调用且必须有 `\| Out-Host`、真跑一段 PowerShell 验「话多的 / 往 stderr 写告警的 / 真失败的」三种子进程都拿到 int、探测连库不许裸重定向、真跑 install.ps1 打一个刚关掉的空端口断言输出里有「连不上数据库」），**两组负向控制**都做了（退回旧写法后 3 条 / 2 条立刻变红，日志直接印证根因：`chatty=Object[]` 而不是 `Int32`、只剩 psql 报错没有提示），随后按 SHA256 逐字节还原。**实装核对**：装到 `%LOCALAPPDATA%\Programs\Have-A-Trip`，6781 个文件、无 `.git`，`backend\.venv\Scripts\python.exe` 与 `frontend\dist\index.html` 都在，`backend\.env` 的 `DATABASE_URL` 指向 `127.0.0.1:55432/attraction_atlas`；库里 1385 个景点、四个 SQL 全部应用成功；从桌面图标真拉起的是**安装副本**（8100 端口的进程命令行落在安装目录，不是仓库），`/api/v1/healthz` 回 `{"status":"ok","database":"ok","version":"0.1.0"}`，`/images/covers/huangshan.jpg`、`/favicon.ico`、`/manifest.json` 全 200，截图确认是新界面。后端用例 363 → **370**（360 通过 + 10 条对拍跳过） |
| 2026-09-26 | v4.10 | **新用例把 CI 打红了: 跳过条件要按「平台」而不是「有没有 powershell」**：v4.9 那 7 条里, 有两条要**真跑** PowerShell —— 一条把 `Invoke-Exe` 抽出来验「话多的 / 往 stderr 写告警的 / 真失败的」三种子进程都拿到 int, 一条真跑 `install.ps1` 打一个刚关掉的空端口, 断言输出里有「连不上数据库」。写的时候想的是「CI 跑在 ubuntu 上、没有 PowerShell, 自然会跳过」, 跳过理由里也就写了「CI 上是常态」。**这个前提是错的**: ubuntu-24.04 那份运行器镜像里装着 PowerShell 7.6.6(顺带还有 PostgreSQL 16.15), 于是 `shutil.which` 在 CI 上照样找得到 `pwsh`, 两条用例真跑了起来 —— 而 harness 第一句就是让 `Invoke-Exe` 去拉 `cmd`, 在 Linux 的 pwsh 上这是一条不存在的命令, 在 `$ErrorActionPreference=Stop` 下直接升级成终止错误, 输出里永远不会出现 `ok=True`; 另一条同理, `install.ps1` 从头到尾都是 Windows 的路径与命令。第一次推送的 `backend-test` 就是这么红的(本地无 admin 权限拿不到 job 日志, 是照着镜像 README 与「不存在的命令会抛终止错误」这段实测反推出来的)。**修法**: 判据换成能力而不是命令名 —— 能不能装这个包, 等价于是不是 Windows, 新增 `_windows_powershell_or_skip()`(`sys.platform != "win32"` 即跳过), 并把这个反例连同镜像版本号写进 docstring; 文本级那几条(BOM / 纯 ASCII / echo 里不许有裸右括号 / `Invoke-Exe` 不许裸调用 / 探测连库不许裸重定向)照旧不分平台, 它们才是 CI 上真正把关的那一层。**验证**: 本机 7 条真跑全过(不是跳过), 另把 `sys.platform` 临时换成 `linux` 单独验了一遍跳过分支确实跳过; 后端全量 360 passed + 10 skipped。教训一句话: **跳过理由里别写「CI 上没有它」—— 先去查那份镜像里到底装了什么** |
| 2026-09-26 | v4.11 | **「AI 生成的图片」换成实拍：查证、换图、把过滤线加厚**：先对齐口径 —— 用户说的「AI 生成图」在仓库里**一张都没有**：全库配图只有两种来源，130 张 Wikimedia Commons 实拍照片，加 `scripts/make_attraction_covers.py` 用代码画的 1255 张 SVG 占位封面（用户看到的就是这批「不是照片」的图），所以这一刀实际做的是**把自绘封面替换成实拍照片**。**踩到的第一个坑**：第一轮 `fetch_commons_photos.py --levels 5A,4A --threads 16` 批量抓回后过目，逮到 9 张配错 —— 西湖配成《漂海录》古籍木刻插图、长白山配成军舰 `PLANS Changbai Mountain (LPD-989)`、陈家祠配成同名地铁站站台、欢乐谷配成「欢乐谷景区」站台、深圳野生动物园配成「Wild Animal Park Station」、东莞展览馆配成天津规划展览馆、国博配成老馆模型、上博与抗战馆配成展厅内景。**根因**：旧过滤器只看文件名里几个词，Commons 的「同名实体」与「非照片媒介」两类噪声拦不住。**修法**：三层判据 —— ① `BAD`（文件名）补「古籍 / 图象 / 木刻」与车站军舰词（`stations?` / `platforms?` / `USS` / `HMS` / `PLANS` / `LPD-\d` / 列车 / 站台）；② 新增类别级 `CATBAD`（`ai-generated` / `stable diffusion` / `midjourney` / `dall-e` / `3d render` / `paintings?` / `drawings?` / `engravings?` / `maps?` / `版画` / `木刻` / `舆图` / `拓片` / `书画` 等，`search()` 为此多取一路 `prop=categories`）；③ 新增 `wrong_place()`，用名录 CSV 的省市词表否掉「别的城市」的图，但**地名若是景点自己名字的一部分就不算错配**（否则「北京市中山公园景区」会被「中山市」误伤）。另修 `api()`：代理会把自家错误页当 200 返回，现在重试 4 次 —— 没这道重试，`--audit` 会把 39 张好图误报成「查不到」。**新增三个运维命令**：`--audit`（按当前口径回查台账，干净退 0 / 可疑退 1）、`--repair`（离线补齐台账缺失字段）、`--drop <file>`（人工过目后剔掉配错的：删台账条目 + 删磁盘图 + 重排 pid）。**结果**：西湖换成 1930 年代《亞細亞大觀》实景老照片（Public domain），长白山 / 上博 / 江门大雁山换到对的图；6 张实在找不到更好来源的（国博 / 抗战馆 / 东莞展览馆 / 陈家祠 / 欢乐谷 / 深圳野生动物园）用 `--drop` 退回自绘 —— **配错比没图更糟**。定稿 `photos.json` **199 条**（实拍 130 → 199，覆盖率 5A 137/373、4A 62/292、3A 仍 0/614），许可分布 BY-SA 151 / BY 40 / CC0+PD 8，share-alike 义务从 98 张涨到 **151 张**（`docs/LICENSE-AUDIT.md` 待办同步到 199）。新增 `backend/tests/test_photo_filter.py` **11 条**守线用例，含负向控制（摘掉 `CATBAD` 后正是「木刻类别」「AI 生成图」两条变红，其余照旧绿）。生成物重跑：1385 张封面 + `db/seed/images.sql`，`--check` 一致，灌库后 `attraction_image` 1385 行 = **199 照片 + 1186 自绘**；后端 **371 passed / 10 skipped**，`verify_all.py` 5/5。**遗留**：库里只有 `credit` / `license` 两列，**没有逐图署名与来源页链接**，CC BY-SA 涨到 151 张后这项义务更重，已在 LICENSE-AUDIT 记为待办 |
| 2026-09-26 | v4.12 | **配图的逐图署名补上了：加一列 `source_url`，声明页从「按作者聚合」变成「一张一行」**：上一轮把 151 张 CC BY-SA 的图收进来时就记着一笔账 —— 库里只有 `credit` / `license` 两列，声明页只能按「作者 + 许可」聚合成几行，**说不清哪一行对应哪张图，也给不出来源页**，而 CC BY / CC BY-SA 要的正是署名、许可链接与来源三样。这一轮把账还了。**库**：`attraction_image` 新增 `source_url`（实拍指向 Commons 文件页，自绘图为 `NULL` —— 不是「来源缺失」，而是它根本没有外部来源这回事），`schema.sql` 里照 `a_level` 的老规矩配一条 `ADD COLUMN IF NOT EXISTS` 升级既有库，并登记 `0005_image_source`；生成器 `make_attraction_covers.py` 把行模板统一成一条 `ROW_TEMPLATE`，新增 `sql_text()`（空值写 `NULL` 不写空串 —— 「没有来源」和「来源是空串」在页面上不是一回事），重跑后 199 行带来源页 / 1186 行 `NULL`。**接口**：`/sources` 新增 `attributions`（只列 `source_url` 非空的行，自绘图不进 —— 没有外部来源就不需要向谁署名）与 `images[].license_url`；新增两个纯函数 `license_url()` 与 `image_modification()`。`license_url()` 认不出来就返回 `None`，页面上那一格只给字面 —— 猜一个链接等于把读者指向别的许可，比没有链接更糟。`image_modification()` 的判据**不是许可而是管线**：抓的是 Commons 重渲染的缩略图、页面上还按版面裁过，所以外部来源的图一律 `modified`（CC BY 3.0 起就要求标注修改，只按 share-alike 判会漏掉 CC BY 那一批），自绘图才是 `not-applicable`。**前端**：`/credits` 新增「图片逐图署名」一张一行 —— 缩略图、`#NNN` 编号、景点（点进站内详情页）、作者、指向许可全文的链接、来源页链接、修改状态；许可那一格抽成 `LicenseCell`，聚合表也跟着能点。缩略图写成 `alt=""` 而把说明文字当链接名 —— 两者都当可访问名的话读屏会念两遍，这是写用例时被 `getByRole("cell")` 的名字里出现两遍编号逼出来的。**守线**：`test_sources.py` 7 → **14 条**（台账每一张都必须有 Commons 来源页；生成的种子里照片行必须带来源页、自绘行必须是 `NULL`；库里出现过的**每一种**许可写法都要能推出全文地址，新增写法忘了登记就红；逐图署名只列有来源的图；下架景点的照片一样不许署上去），`db-schema.yml` 加 4 条断言（199 / 自绘不许带来源 / 实拍不许缺来源 / 0005 已登记）与一条自愈（把 `source_url` 清空后重跑 `images.sql` 要修回来）。**验证**：本地 PG 应用 schema 与种子（ALTER 补列成功，199 与 1186 分列，清空后重跑确实修回），Edge 截图确认页面渲染；后端 **371 → 378 passed** / 10 skipped，前端 **160 passed**，`verify_all.py` 5/5，`license_gate.py --strict` exit 0。**遗留**：许可全文链接是按文本推的，新增许可写法要在 `LICENSE_URLS` 或解析分支里补一处（有守线拦着）；相同方式共享的义务本身没变 —— CC BY-SA 的衍生版本仍要按 CC BY-SA 分发 |
| 2026-09-26 | v4.13 | **三条工作流里的 action 升到 v7: 清掉「Node.js 20 已弃用」那条一直在跑的通知**: 起因是收到一封 Actions 邮件「后端测试: 所有工作都失败了」, 查下去发现**红的那次是旧的** —— `f17aa5c` 那次运行(本地 17:42)挂在第 6 步「跑测试(含 db/schema.sql 对拍)」并把第 7 步跳过了, 而 `7a64a84` 五分钟后就把根因修好了, 最近 30 次运行里只有那一次是红的, `main` 一直全绿 —— 邮件的红叉不是新问题。**但这趟查证顺手看见一条一直没人管的 annotation**: 每次运行都挂着`Node.js 20 is deprecated ... actions/checkout@v4, actions/setup-python@v5`(backend-test)与 `... actions/checkout@v4, actions/setup-node@v4`(frontend-build)。它不影响结论, 属于「看得见但没人看」的黄条, 于是把这笔也还了。**改动**: 5 份 workflow 共 9 处引用, `actions/checkout@v4` → **v7**、`actions/setup-python@v5` → **v7**、`actions/setup-node@v4` → **v7**; 动手前逐个读上游`action.yml` 确认三者的 `runs.using` 都是 **`node24`**(checkout v7 发布于 2026-06-18、setup-python v7 2026-07-20、setup-node v7 2026-07-14 —— 顺带说明: 原以为的「升到 v5 / v6」已经落后两个大版本, 先查再改)。**破例点逐条核对, 与本仓库用法无关**: checkout 从 v6 起把凭据写到单独文件、v7 起在 `pull_request_target` / `workflow_run` 下禁止 checkout fork PR(本仓库只有 `push` 与 `pull_request`, CI 里也不 push); setup-python v7 删掉了 `pip-install` 入参(没用到, 用的是 `cache: pip` + `cache-dependency-path`); 三个 action 的 v5/v6/v7 都要求 runner ≥ `2.327.1`, GitHub 托管运行器远高于此。**验证**: 本地按 workflow 自己那段脚本解析 5 份 YAML(合起来 9 处 `uses` 全是 v7)、`license_gate.py --strict` exit 0; 推送后 5 条工作流全绿, 两条 annotation 里的 Node 20 警告消失。**留观**: 同日另一条通知是 `ubuntu-latest` 将于 2026-10-19 迁到 Ubuntu 26 —— 那是标签自动迁移, 这次不动它, 真迁了再看要不要钉 `ubuntu-24.04` |
| 2026-09-26 | v4.14 | **一张封面一直在裂图: 种子里拼 URL 时把后缀写死了**：这轮本来是「检测所有 AI 绘图的景区」（离线取证扫了 `frontend/public/` 下 1589 个图片资产 —— SVG 看自绘标记 / 生成器注释 / 内嵌位图, JPEG 看 EXIF / XMP / `digitalSourceType=trainedAlgorithmicMedia` / C2PA, PNG 看 `tEXt` / `caBX`, 再加台账字段的关键词, **结论: AI 生成图 0 张**; 真正的现状是 1385 张脚本自绘 SVG + 199 张 Commons 实拍), 顺手做「种子里的每个配图 URL 在磁盘上是不是同一个文件」时逮到这一个 —— 珠海市圆明新园(`4a-guangdong-178`)在库与种子里指的 `/images/covers/4a-guangdong-178.jpg` **磁盘上根本不存在**, 真文件叫 `.jpeg`(253,625 字节)。**为什么一直没被发现**: `backend/app/web.py` 对找不到的静态路径会回落 SPA 外壳, 那个 URL 的响应是 **200 `text/html`**(2379 字节, 正好是 `frontend/dist/index.html` 的大小) —— 给 `<img>` 喂回去一段 HTML, 页面上只是那张图空着, 连 404 都没有。**根因**: 两个脚本对后缀的约定不一致 —— `fetch_commons_photos.py` 是**故意保留原图真实后缀**的(`.jpg` / `.jpeg` / `.png`, 注释写着「缩略图地址的后缀就是真实格式」), 台账 `photos.json` 记的也是真实文件名; 而 `make_attraction_covers.py` 的 `PHOTO_URL_TEMPLATE = "/images/covers/{slug}.jpg"` 拿 slug 另拼了一个。199 张里 198 张恰好是 `.jpg`, 所以只漏这一张。**三道自检为什么都放过了**: `fetch --check` 查的是「台账里的文件在不在盘上」(在); `make_attraction_covers --check` 查的是「SVG 内容 + `images.sql` 内容 + 台账照片在不在盘上」(都在); db-schema 的配图断言用的是 `url like '%.svg'` / `source_url is not null`(`.jpg` 同样算实拍)—— 全都在查「台账 ↔ 磁盘」, 没有一条查「种子里的 URL ↔ 台账」。**修法**: `PHOTO_URL_TEMPLATE` 换成 `PHOTO_URL_PREFIX`, URL 直接取台账里的 `photo`, 台账成为唯一源(与「CSV 是唯一源」同一条口径); 并在 `--check` 里加一条断言 —— 种子 VALUES 行里的每个 `/images/covers/...` 都要在 `COVER_DIR` 里找得到同名文件, 且认出来的条数必须等于景点数(不写后半句, 正则有天匹配不到就成了空转)。**守线**: `test_sources.py` 14 → **16 条**(种子里每个配图 URL 在磁盘上存在; 照片行的 URL 必须等于台账 `photo`, 后缀只能是抓取脚本肯写的那三个)。**负向控制两组都做了**: 把 `PHOTO_URL_TEMPLATE` 放回去重跑 `--check` → 立刻报「种子里的 `/images/covers/4a-guangdong-178.jpg` 在磁盘上不存在」并 exit 1; 同一状态跑 pytest → 两条新用例红、其余 14 条照旧绿; 随后把脚本与 `images.sql` 按 SHA256 逐字节还原(脚本 `5990e404…`、种子 `fd190128…`)。**验证**: 重跑生成器只有 1 行变化(那一条 URL), 1385 张 SVG 逐字节没动; 本机库重灌 `images.sql` → 1385 行、该景点 `cover_image` 与 `attraction_image.url` 都变成 `.jpeg`、没留下旧 `.jpg` 行; 起临时实例实测 `/api/v1/attractions/4a-guangdong-178` 的 `cover_image` = `.jpeg`, 该 URL 200 `image/jpeg` 253,625 字节, Edge 截图确认封面真的画出来了; 后端 **378 → 380 passed** / 10 skipped, `verify_all.py` 5/5, `license_gate.py --strict` exit 0 |
