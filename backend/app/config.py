...
class Settings(BaseSettings):
    ...
    # ---------------------------------------------------------------- 用户动态
    #
    # 动态是**唯一**由用户产出自由文本的地方, 所以限额不是可选项: 没有账号体系时,
    # 一个脚本能把整屏刷满。计数与行程共用一张计数器表, 但 owner_key 带 post: 前缀,
    # 两边的名额互不影响 —— 见 app/quota.py。
    #
    # 每个 owner 每天能发几条。0 表示不限(只建议本地开发时这么配)
    post_daily_limit: int = 10
