...
def nearby_attractions(
    ...
    第三级是「国内」而不是「全部」: 库里 1385 条已发布里有 40 条在境外、摊在 30 来个国家
    (见 /stats 的 by_country)。名录条目进来之后境外只占 3% 左右, 一次要三个约 8% 的概率
    会掺进境外景点 —— 概率是变小了, 但把首页第一屏的「出去走走」推成一张去加拿大的机票
    仍然说不通, 所以这一级照旧收口。反过来, 也不为已知在境外的访客按国别抽: 那边每个
    国家只有一两条, 抽出来等于「就那一条」。
